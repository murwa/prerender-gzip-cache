...
module.exports = {
    beforePhantomRequest: function(req, res, next) {
        ...
        client.get(req.prerender.url, function(err, result) {
            ...
            if (!err && result) {
                // Inflate
                res.send(200, zlib.inflateSync(new Buffer(result, 'base64')).toString());
            } else {
                next();
            }
        });
    ...
    afterPhantomRequest: function(req, res, next) {
        ...
        if (req.prerender.statusCode === 200) {
            // Minify
            // We simply remove white space for now
            // https://github.com/kangax/html-minifier#options-quick-reference
            var _doc = htmlMinify(req.prerender.documentHTML, {
                collapseWhitespace: true
            });
            // Deflate document
            client.set(req.prerender.url, zlib.deflateSync(_doc).toString('base64'), function(err, reply) {
